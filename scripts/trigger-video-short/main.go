...
func main() {
	...
	// Data dummy GoldScrapedEvent untuk men-trigger video-short-generator
	...
	err := q.Publish(queue.KeyVideoShortTrigger, scrapedEvent)
	if err != nil {
		log.Fatalf("❌ Failed to publish to %s: %v", queue.KeyVideoShortTrigger, err)
	}

	data, _ := json.MarshalIndent(scrapedEvent, "", "  ")
	fmt.Printf("📥 Pushed to %s:\n%s\n\n", queue.KeyVideoShortTrigger, string(data))
	log.Println("✅ Dummy gold.scraped event pushed to video.short.trigger queue.")
}
